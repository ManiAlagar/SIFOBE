...
namespace SIFO.APIService.Hospital.Repository.Implementations
{
    public class HospitalRepository : IHospitalRepository
    {
        ...
        //public async Task<bool> CreatePharmacyAsync(HospitalPharmacyRequest pharmacy, long hospitalId)
        //{
        //    try
        //    {
        //        var data = await _commonService.GetDataFromToken();
        ...
        public async Task<Dictionary<string, List<CalendarResponse>>> GetCalendarByIdAsync(long pharmacyId, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                foreach (var date in dateRange)
                {
                    ...
                    if (calendarResult.Any(a => a.CalendarDate.Value.ToString("MM-dd-yyyy") == key))
                    {
                        response[key] = calendarResult.Where(a => a.CalendarDate.Value.ToString("MM-dd-yyyy") == key).ToList();
                    }
                    else
                    {
