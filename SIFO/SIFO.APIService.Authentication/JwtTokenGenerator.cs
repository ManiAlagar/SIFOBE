...
namespace SIFO.APIService.Authentication
{
    public class JwtTokenGenerator
    {
        ...
        public string GenerateTokenForPatient(PatientsLoginResponse patient)
        {
            var signingCredentials = new SigningCredentials(
                new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_jwtSettings.Secret)),
                SecurityAlgorithms.HmacSha256
            ...
            List<Claim> claims = new();
            Claim Email = new Claim("Email", patient.Email);
            Claim UserId = new Claim("UserId", patient.UserId.ToString());
            Claim RoleId = new Claim("RoleId", patient.RoleId.ToString());
            Claim UserName = new Claim("UserName", $"{patient.FirstName} {patient.LastName}");
            Claim Roles = new Claim(ClaimTypes.Role, patient.RoleName);
            claims.Add(Email);
            claims.Add(UserId);
            claims.Add(UserName);
            claims.Add(Roles);
