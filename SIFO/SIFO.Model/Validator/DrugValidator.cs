        ...
        public DrugValidator()
        {
            ...
            When(x => x.DD || x.DPC, () =>
            {
                RuleFor(x => x.ProductType.Trim())
                    .NotEmpty().WithMessage("ProductType is required when DD or DPC is true.");

                RuleFor(x => x.Class.Trim())
                    .NotEmpty().WithMessage("Class is required when DD or DPC is true.");

                RuleFor(x => x.PharmaceuticalForm.Trim())
                    .NotEmpty().WithMessage("PharmaceuticalForm is required when DD or DPC is true.");

                RuleFor(x => x.UMR)
                    .NotEmpty().WithMessage("UMR is required when DD or DPC is true.")
                    .GreaterThan(0).WithMessage("UMR must be greater than zero.");

                RuleFor(x => x.PrescriptionType.Trim())
                    .NotEmpty().WithMessage("PrescriptionType is required when DD or DPC is true.");

                RuleFor(x => x.ProductImage.Trim())
                    .NotEmpty().WithMessage("ProductImage is required when DD or DPC is true.");

                RuleFor(x => x.TherapeuticIndications.Trim())
                    .NotEmpty().WithMessage("TherapeuticIndications are required when DD or DPC is true.");

                RuleFor(x => x.Temperature.Trim())
                    .NotEmpty().WithMessage("Temperature is required when DD or DPC is true.");
