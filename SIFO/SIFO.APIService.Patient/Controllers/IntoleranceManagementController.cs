...
namespace SIFO.APIService.Patient.Controllers
{
    ...
    public class IntoleranceManagementController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllIntoleranceManagementAsync([FromHeader] int pageNo = 1, [FromHeader] int pageSize = 10, [FromHeader] string filter = "", [FromHeader] string sortColumn = "Id", [FromHeader] string sortDirection = "DESC", [FromHeader] bool isAll = false , [FromHeader] long patientId = 0)
        {
            try
            {
                var result = await _intoleranceManagementService.GetAllIntoleranceManagementAsync(pageNo, pageSize, filter, sortColumn, sortDirection, isAll,patientId);
        ...
        [ProducesResponseType(typeof(ApiResponse<string>), StatusCodes.Status404NotFound)]
        [ProducesResponseType(typeof(ApiResponse<string>), StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> UpdateIntoleranceManagementAsync(IntoleranceManagementRequest request, [FromRoute] long intoleranceManagementId, [FromHeader] long PatientId, [FromHeader] long? AuthenticationType, [FromHeader] string? AuthenticationFor, [FromHeader] string? OtpCode)
        {
            try
            {
                var validationResult = await _intoleranceManagementValidator.ValidateAsync(request);
                ...
                request.Id = intoleranceManagementId;
